...
(function ($) {
  ...
  ==========================*/
    ...
    $(window).on("scroll", function () {
        ...
        if (scroll < 100) {
            $(".love-sticky:not(.blog-details)").removeClass("sticky fadeInDown animated");
            headerMain.removeClass("sticky fadeInUp animated");
        } else {
            $(".love-sticky:not(.blog-details)").addClass("sticky fadeInDown animated");
        }
    });
  ...
  ====================================*/
    var $swiper = $(".swiper");
    var $swiperScreenshot = $(".screenshot-slider");
    var categorySlider = [];
    ...
    $swiperScreenshot.each(function () {
        var $t = $(this);
        var slider = new Swiper($t[0], {
            slidesPerView: checkData($t.data("swiper-items"), 1),
            spaceBetween: checkData($t.data("swiper-margin"), 50),
            loop: checkData($t.data("swiper-loop"), true),
            centeredSlides: true,
            speed: checkData($t.data("swiper-speed"), 1000),
            grabCursor: checkData($t.data("swiper-grab-cursor"), true),
            ...
            autoplay: {
                delay: 1000,
                disableOnInteraction: false, // Allow manual interactions to stop autoplay temporarily
            },
            on: {
                slideChange: function () {
                    // Center the active slide immediately on slide change
                    centerActiveSlide(this);
                },
                transitionEnd: function () {
                    // Center the active slide after transition ends
                    centerActiveSlide(this);
                },
                resize: function () {
                    // Center the active slide after window resize
                    centerActiveSlide(this);
                },
                touchEnd: function () {
                    // Center the slide after manual drag (touch or mouse)
        ...
        // Center active slide function
        function centerActiveSlide(swiper) {
            const activeSlide = swiper.slides[swiper.activeIndex];
            const offset = (swiper.width - activeSlide.offsetWidth) / 2;
            swiper.wrapperEl.style.transform = `translate3d(-${
                activeSlide.offsetLeft - offset
            }px, 0, 0)`;
        }
    ...
    // ========== autoWheel swiper slider starts
    let autoWheelSwiper;

    function initializeAutoWheelSwiper() {
        if (autoWheelSwiper) {
            autoWheelSwiper.destroy(true, true);
        }

        autoWheelSwiper = new Swiper(".autoWheelSwiper", {
            loop: true,
            autoHeight: false,
            autoplay: {
                delay: 4000,
  ...
  ====================================*/
    ...
    /*==================================
  18: Blog Details Article mobile menu
  ====================================*/
    $('.article-menu-button').on('click', function() {
        $(this).toggleClass('show');
        $('.in-this-article').toggleClass('show');
    })
