    ...
    <div class="content container-fluid">
        <div class="d-flex justify-content-between gap-2 align-items-center">
            <div class="d-flex align-items-center gap-3 pb-2">
                <h2 class="page-header-title">{{translate('Edit Blog')}} ({{ translate('ID'). ' # '. $blog->readable_id }})</h2>
            </div>
            <a class="btn btn-outline-primary" href="{{ route('admin.blog.preview', $blog->id) }}" target="_blank">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="writer" class="form-control" placeholder="First Name" value="{{ $blog->writer }}" maxlength="100">
                            </div>
                            <div class="form-group">
                                <label class="input-label">{{ translate('publish_Date') }}</label>
                                <input type="date" name="publish_date" value="{{ date('Y-m-d', strtotime($blog->publish_date)) }}" class="form-control">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                <div class="d-flex flex-column align-items-center text-center">
                                    ...
                                    <div class="upload-file auto profile-image-upload-file">
                                        ...
                                        <div class="upload-file__img banner border-gray d-flex justify-content-center align-items-center mw-100 w-360 h-180 p-0 bg-light">
                                            <div class="upload-file__textbox text-center" @if($blog->image) style="display:none;" @endif>
                                            ...
                                            <img class="upload-file__img__img h-100 ratio-2" width="180" height="180" @if($blog->image) style="display: block" @endif src="{{ $blog['image_fullpath'] }}" loading="lazy" alt="">
                                        </div>
                    ...
                    <div class="mt-3 p-4 bg-soft-secondary rounded">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" class="form-control" value="{{ $blog->title }}" placeholder="{{translate('title')}}" maxlength="255" required>
                        </div>

                        <div class="">
                            <label class="input-label" for="title">{{translate('description')}}<span class="text-danger"> *</span></label>
                            <div id="editor" class="min-h200 bg-white">{!! $blog->description !!}</div>
                            <textarea class="editor form-control d-none" name="description" id="hiddenArea">{!! $blog->description !!}</textarea>
                        </div>
                    </div>
