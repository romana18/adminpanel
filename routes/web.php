...
use App\Http\Controllers\Gateway\CashFreePaymentController;
use App\Http\Controllers\Gateway\PhonepeController;
use App\Http\Controllers\Gateway\InstamojoPaymentController;
...
if (!addon_published_status('Gateways')) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'phonepe', 'as' => 'phonepe.'], function () {
            ...
            Route::any('redirect', [PhonepeController::class, 'redirect'])->name('redirect')->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });

        Route::group(['prefix' => 'cashfree', 'as' => 'cashfree.'], function () {
            Route::any('pay', [CashFreePaymentController::class, 'payment'])->name('pay');
            Route::any('callback', [CashFreePaymentController::class, 'callback'])->name('payment')->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });

        Route::group(['prefix' => 'instamojo', 'as' => 'instamojo.'], function () {
            Route::any('pay', [InstamojoPaymentController::class, 'payment'])->name('pay');
            Route::any('callback', [InstamojoPaymentController::class, 'callback'])->name('payment')->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });
